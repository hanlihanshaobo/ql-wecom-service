...
- **应用菜单**：启动后自动创建底部固定菜单，点击即可操作（任务/订阅/脚本/依赖/变量操作、发送通知、查看配置等）
...
按资源类型分类：**任务**（任务操作）、**资源**（订阅/脚本/依赖/变量/自定义脚本）、**系统**（系统操作/日志/通知/配置/帮助）。点击带"操作"的按钮后，会先显示对应列表，并附上全部操作指令提示，只需回复如 `执行 1`、`日志 1`、`禁用任务 1`、`运行订阅 网易云`、`删变量 3`、`日志详情 <文件名>` 即可操作，无需输入完整名称。
